...
inline int qry(int L,int R){int t=Log[R-L+1];return cmp(f[L][t],f[R-Mi[t]+
inline ll left(int L,int R){int p=qry(L-1,R);return (ll)a[p]*(R-p+1)+fl[L
inline ll right(int L,int R){int p=qry(L,R+1);return (ll)a[p]*(p-L+1)+fr[R+
int main(){
	#ifndef ONLINE_JUDGE
	file("s");
	#endif
	...
	fp(i,1,m){
		x=q[i].l,y=q[i].r;
		while(L>x)Now+=left(L--,R);
		while(R<y)Now+=right(L,R++);
		while(L<x)Now-=left(++L,R);
		while(R>y)Now-=right(L,--R);
		ans[q[i].id]=Now;
	}
