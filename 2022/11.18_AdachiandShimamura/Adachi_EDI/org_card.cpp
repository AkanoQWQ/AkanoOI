...
EXCUTE_ST
	...
	for(int i = 2;i <= n;i++){
		...
		for(int j = l;j < i;j++){
			fs poss = a[j];
			cout<<"ORGPOSS"<<++ccc<<" "<<poss.up * ksm(poss.down,MOD-2) % MOD<<endl;
			int m = 1;
			for(int s = j + 1;s < i;s++){
				cout<<"TEST"<<++ccc<<" "<<poss.up * ksm(poss.down,MOD-2) % MOD<<endl;
				poss = poss * p[m].f();
				cout<<"TEST"<<++ccc<<" "<<poss.up * ksm(poss.down,MOD-2) % MOD<<endl;
				++m;
			}
		...
		cout<<"prea["<<i<<"]:"<<a[i].up * ksm(a[i].down,MOD-2)%MOD<<endl;
		cout<<nonek.up * ksm(nonek.down,MOD-2) % MOD<<" * "<<p[t].up *  ksm(p[t].down,MOD-2) % MOD<<endl;
		a[i] = a[i] + (nonek * (p[t]));
		cout<<"a["<<i<<"]:"<<a[i].up * ksm(a[i].down,MOD-2)%MOD<<endl;
	}
	for(int i = 1;i <= n;i++){
