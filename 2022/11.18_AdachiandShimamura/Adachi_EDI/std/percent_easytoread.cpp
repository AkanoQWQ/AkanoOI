...
char s[N];
int n,q,stk[N],top,stk0[N],top0,cnt,t[N],rt[N],pare[N][M],mins[N][M][2][2],snum[N],dep[N],rk[N];
vector<int> to[N];
...
inline int fnd(int u,int tu,int v,int tv) {
	...
	if(rt[u]==rt[v]){
		...
		int ans;
		if(u<v){
			ans=(rk[v]-rk[u]-1)*2+up_u[1]+up_v[0]+1;
		}
		else{
			ans=(rk[u]-rk[v]-1)*2+up_v[1]+up_u[0]+1;
		}
		{
	...
	if(u<v){
		return (rk[v]-rk[u]-1)*2+up_u[1]+up_v[0]+1;
	}
...
signed main() {
	...
	for(int i=1;i<=n;i++){
		if(s[i]=='('){
			...
			t[i]=t[stk[top]];
			while(top0&&stk0[top0]>stk[top]){
				to[t[stk[top]]].push_back(t[stk0[top0--]]),++snum[t[stk[top]]];
			}
			stk0[++top0]=stk[top--];
		}
	}
