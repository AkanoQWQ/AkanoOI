...
int query(int rt,int l,int r,int L,int R){
    if(segtree[rt].l == L){
    	Lc=segtree[rt].lc;  
//    	cout<<"in "<<rt<<"read "<<L<<" "<<Lc<<endl;
	} 
    if(segtree[rt].r == R) {
    	Rc=segtree[rt].rc;  
//    	cout<<"en "<<rt<<"read "<<R<<" "<<Rc<<endl;
	}
    if(segtree[rt].l == l && segtree[rt].r == r)  
    ...
    int mid=(segtree[rt].l+segtree[rt].r)>>1;  
    if(rt == 4){
//		cout<<segtree[L(rt)].rc<<" compare "<<segtree[R(rt)].lc<<endl;
	}
    if(r<=mid)  
...
//            cout<<u<<","<<v<<" query"<<endl;
//            cout<<pos[top[u]]<<","<<pos[u]<<" query_seg"<<endl;
