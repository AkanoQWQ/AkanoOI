...
int judge(int px1,int py1,int px2,int py2){
	ll fd = 1ll * s[px2 * (!swp) + py2 * swp][py2 * (!swp) + px2 * swp] + s[(px1-1) * (!swp) + (py1-1) * swp][(py1-1) * (!swp) + (px1-1) * (swp)] - s[(px1-1) * (!swp) + py2 * swp][py2 * (!swp) + (px1-1) * (swp)] - s[px2 * (!swp) + (py1-1) * swp][(py1-1) * (!swp) + px2 * (swp)];
...
int calc(int upp,int dwn){
	int l = 1,r = 1,ret = 0;//91 92
	while(r <= m && l <= m){
		int res = judge(upp,l,dwn,r),orgr = r;
		bool addl = 0;
		if(res == -1){
			r++;
		}else if(res == 1){
