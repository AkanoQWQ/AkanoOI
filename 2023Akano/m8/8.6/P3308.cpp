...
int T,nnt,VST,VED,no[MAXE][2],lis,eno[MAXE];
inline void BuildEdge(){
	...
	for(int i = 1;i <= n;i++){
		...
		eno[i] = cnt-1;
		for(int j = i+1;j <= n;j++){
			if(arr[i].step+1 == arr[j].step)CreateEdge(no[i][1],no[j][0],INF);
		}
	}
	return ;
}
...
int main(){
	...
	while(T--){
		...
		BuildEdge();
		ll nowans = Dinic(nnt,VST,VED);
		vector<int> votp;
		for(int i = 1;i <= n;i++){
			const int u = dir[i];
			if(e[eno[u]].cap == 0 && !BFS(nnt,no[u][0],no[u][1])){
				Dinic(nnt,VED,no[u][1]);
				Dinic(nnt,no[u][0],VST);
				votp.push_back(u);
			}
		}
		sort(votp.begin(),votp.end());
		Write(nowans),putchar(' '),Write(votp.size()),putchar('\n');
		for(auto i : votp){
			Write(i),putchar(' ');
		}
		putchar('\n');
