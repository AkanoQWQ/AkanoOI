...
Bigint t,n,m;
inline Bigint Sum(Bigint l,Bigint r){//l,r <= 1e18
	if(l > r)return 0;
	return (r - l + 1) * (l + r) / 2;
}
inline Bigint Calc(Bigint x){
	if(x <= 0)return -1;
	if(x-1 > Sum(1,m)){
		return -1;
	}
	Bigint nR = Sum(x,x+n-1);
	Bigint mR;
	if(x > m){
		mR = Sum(1,m);
	}else{
		mR = Sum(1,x-1) + Sum(nR+1,nR+1+m-(x-1)-1);
	}
	return max(nR+1,mR+1);
}
inline Bigint Solve(){
	Bigint ret = 0;
	for(Bigint i = 1;i <= Sum(1,m) + 1;i++){
		ret = max(ret,Calc(i));
	}
	return ret;
}
