...
//	freopen("fish/fish3.in","r",stdin);
	...
	for(auto i : udm){
		...
		res -= i.second;
//		cerr<<i.second<<" add "<<((1ll * i.second * res) % MOD) * case3<<" adn "<<((1ll * i.second * (i.second-1)) % MOD) * case2<<" res "<<res<<endl;
		ans = (ans + ((1ll * i.second * res) % MOD) * case2) % MOD;
		if(i.first != 0){
			ans = (ans + ((2ll * i.second * i.second) % MOD) * case3) % MOD;
		}
	}
	cout<<ans<<endl;
