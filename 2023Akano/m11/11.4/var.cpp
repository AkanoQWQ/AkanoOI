...
public:
	...
	inline Node& BackNext(){
		if(reversed){
			return content[1];
		}
		return content[content.size() - 2];
	}
	inline Node& FrontNext(){
		if(reversed){
			return content[content.size() - 2];
		}
		return content[1];
	}
	inline void PushFront(const Node& val){
		if(reversed){
	...
	inline void PushBack(const Node& val){
		if(reversed){
			content.push_front(val);
		}
		content.push_back(val);
	...
	inline void PushDownFront(){
		Front().PushDown();
		return ;
	}
	inline void PushDownBack(){
		Back().PushDown();
		return ;
	}
