...
constexpr ll INF = 1e15 + 200610181108;
...
int n,m,A,B,C,D;
ll adis[MAXN],cdis[MAXN],ddis[MAXN];
vi apre[MAXN],cpre[MAXN],dpre[MAXN];
ll cmin[MAXN],dmin[MAXN];
bool vis[MAXN];
inline void Dij(ll dis[],vi pre[],int st){
	for(int i = 1;i <= n;i++){
		dis[i] = INF;
		vis[i] = false;
	}
...
inline void BFS(int ST){
	...
	while(!q.empty()){
		...
		q.pop();
		for(auto v : apre[u]){
			cmin[v] = min(cmin[v],cmin[u]),dmin[v] = min(dmin[v],dmin[u]);
			if(!vis[v]){
				q.push(v),vis[v] = true;
			}
		}
...
int main(){
	...
	Dij(adis,apre,A),Dij(cdis,cpre,C),Dij(ddis,dpre,D);
	BFS(B);
	ll ans = INF;
	for(int i = 1;i <= n;i++){
		ans = min(ans,cdis[i] + dmin[i]);
		ans = min(ans,ddis[i] + cmin[i]);
	}
	fio<<ans<<endl;
