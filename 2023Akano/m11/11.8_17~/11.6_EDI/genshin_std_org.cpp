...
void Add(int u, int to) {
	while(top[u] != top[to])add(dfn[top[u]], dfn[u], 1), u = fa[top[u]];
	add(dfn[to], dfn[u], 1);
}
...
void dfs(int u) {
	for(auto i : v[u])i != fa[u] && (dfs(i), f[u][0] += max(f[i][0], f[i][1]));
...
void DFS1(int u) {
	...
	for(auto i : v[u])i != fa[u] && (DFS1(i), 0);
}
int DFS2(int u) {
	int res = op[u] == 2;
	for(auto i : v[u])i != fa[u] && (res += DFS2(i));
	return op[fa[u]] == 2 ? res : (asw += 1ll * res * res, 0);
}
int main() {
	freopen("genshin/genshin1.in","r",stdin);
	scanf("%d%d", &n, &m);
	for(int x, y, i = 1; i < n; i++)scanf("%d%d", &x, &y), v[x].push_back(y), v[y].push_back(x);
	for(int i = 1; i <= m; i++)scanf("%d%d", qx + i, qy + i);
	dfs1(1, 0), dfs2(1, 1);
	for(int i = 1; i <= m; i++)p[lca(qx[i], qy[i])].push_back(i);
	dfs(1), memset(s, 0, sizeof(s));
	op[1] = 1, DFS1(1), DFS2(1), printf("%lld", asw);
}
