...
class STtable{
private:
	int mn[MAXN * 3][24],logval[MAXN * 3];
public:
	...
	inline int Query(int l,int r){
		...
		return min(mn[l][lgval],mn[r-(1<<lgval)+1][lgval]);
	}
}st;
inline bool Able(int l,int r,const vector<pii>& ban){
	for(auto i : ban){
		if(i == make_pair(l,r))continue;
		int nowl = max(l+1,i.first),nowr = min(r-1,i.second);
		if(nowl <= nowr){
//			cerr<<l<<","<<r<<"fail becuae "<<i.first<<","<<i.second<<endl;
			return false;
		}
	}
	
	assert(st.Query(l+1,r-1) != r);
	return st.Query(l+1,r-1) > r;
}
inline ll Solve(int delL,int delR,bool throughCenter){
	...
	for(int i = 1;i < n;i++){
		...
		if(int(arrpos[i].size()) == 2){
			ban.emplace_back(arrpos[i][0],arrpos[i][1]);
			farpos[arrpos[i][0]] = min(farpos[arrpos[i][0]],arrpos[i][1]);
		}else{
			ban.emplace_back(arrpos[i][1],arrpos[i][1]);
			farpos[arrpos[i][0]] = min(farpos[arrpos[i][0]],arrpos[i][1]);
			farpos[arrpos[i][1]] = min(farpos[arrpos[i][1]],arrpos[i][2]);
		}
	}
	st.Build(tail);
	...
	for(int i = 1;i <= tail;i++){
		const int x = arr[i];
		if(Stat3(i)){
			if(Able(arrpos[x][0],arrpos[x][1],ban)){
				f[arrpos[x][1]+1] += f[i];
				ferr<<i<<"update"<<arrpos[x][1]+1<<" case1-1"<<endl;
			}else{
				ferr<<i<<" try upd "<<arrpos[x][1]+1<<" fail"<<endl;
			}
			f[i+1] = (f[i+1] + f[i]) % MOD;
			ferr<<i<<"update"<<i+1<<" case1-2"<<endl;
		}else if(Stat2(i)){
			if(int(arrpos[x].size()) == 2){
				if(Able(arrpos[x][0],arrpos[x][1],ban)){
					ferr<<i<<"update"<<arrpos[x][1]+1<<" case2-1"<<endl;
					f[arrpos[x][1]+1] = (f[arrpos[x][1]+1] + f[i]) % MOD;
				}
			}else{
				if(Able(arrpos[x][1],arrpos[x][2],ban)){
					ferr<<i<<"update"<<arrpos[x][2]+1<<" case2-2"<<endl;
					f[arrpos[x][2]+1] = (f[arrpos[x][2]+1] + f[i]) % MOD;
				}
			}
		}else if(Stat1(i)){
			f[i+1] = (f[i+1] + f[i]) % MOD;
			ferr<<i<<"update"<<i+1<<" case3"<<endl;
		}
	}
//	for(int i = 1;i <= tail;i++){
//		cerr<<f[i]<<" ";
...
//	ferr("memory usage : {} MiB\n",(&_ST_ - &_ED_) / 1024. / 1024.);
	freopen("circle/circle1.in","r",stdin);
