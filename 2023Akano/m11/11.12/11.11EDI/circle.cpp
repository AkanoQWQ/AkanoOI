...
int segarr[MAXN * 3];
inline bool Able(int l,int r,bool w){
	return segarr[r-1] - segarr[l+1-1] <= w * (r-1-l-1+1);
}
inline ll Solve(int delL,int delR,bool throughCenter){
	...
	for(int i = 1;i < n;i++){
		...
		if(int(arrpos[i].size()) == 2){
			segarr[arrpos[i][0]]++,segarr[arrpos[i][1]+1]--;
		}else{
			segarr[arrpos[i][1]]++,segarr[arrpos[i][1]+1]--;
		}
	}
	for(int i = 1;i <= tail;i++){
		segarr[i] += segarr[i-1];
	...
	for(int i = 1;i <= tail;i++){
		const int x = arr[i];
		if(Stat3(i)){
			if(Able(arrpos[x][0],arrpos[x][1],false)){
				f[arrpos[x][1]+1] += f[i];
			}
			f[i+1] = (f[i+1] + f[i]) % MOD;
		}else if(Stat2(i)){
			if(int(arrpos[x].size()) == 2){
				if(Able(arrpos[x][0],arrpos[x][1],true)){
					f[arrpos[x][1]+1] = (f[arrpos[x][1]+1] + f[i]) % MOD;
				}
			}else{
				if(Able(arrpos[x][1],arrpos[x][2],false)){
