...
//	freopen("easy.out","w",stdout);
	...
	for(auto nowuv : uv){
		...
		int dfnl = u[dfn],dfnr = u[dfn] + u[siz] - 1,delta = 0;
		for(int i = 1;i <= n;i++){
			if(dfnl <= i && i <= dfnr)continue;
			int now = tr[i].Query(dfnl,dfnr),len = dfnr - dfnl + 1;
			delta += now - (len - now);
		}
		if(delta >= 0){
			fliped[++uvcnt] = true;
			...
			for(int i = 1;i <= n;i++){
				if(dfnl <= i && i <= dfnr){
					tr[i].Change(1,dfnl-1),tr[i].Change(dfnr+1,n);
				}else{
					tr[i].Change(dfnl,dfnr);
				}
