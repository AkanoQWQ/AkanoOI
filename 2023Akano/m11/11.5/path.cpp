...
ll ans,m,n;
bool Cmp4Edge(Rect rec1,Rect rec2){
	return rec1.v.y < rec2.v.y;
}
template<typename T>
inline void DoMod(T& val){
	val = ((val % MOD) + MOD) % MOD;
	return ;
}
inline ll Solve(vector<Rect> edge){//不引用也不会有太多开销吧......
	sort(edge.begin(),edge.end(),Cmp4Edge);
	ll ret = 0;
	...
	for(int i = 0;i < a;i++){
		for(int j = 0;j <= siz+2;j++)f[j] = -INF;
		int turn = 0;
		for(int j = b-1;j >= 0;j--){
			ll nowsize = ll(xvec[i+1] - xvec[i]) * (yvec[j+1] - yvec[j]);
			DoMod(nowsize);
			...
			for(int k = 0;k < turn;k++){
				...
				for(auto p : save[k]){
					delta += ll(n + 1 - farpos) * (yvec[edge[p].v.y] - lst);
					DoMod(delta);
					lst = yvec[edge[p].v.y];
					farpos = min(farpos,ll(xvec[edge[p].v.x]));
				}
				delta += ll(n + 1 - farpos) * (n + 1 - lst);
				DoMod(delta);
				ret += delta * nowsize;
				DoMod(ret);
			}
