...
class STtable{
private:
	ll val[MAXN][21],logval[MAXN];
public:
	inline void ReadIn(int x){
		for(int i = 1;i <= x;i++){
			if(i > 1)logval[i] = logval[i / 2] + 1;
			fio>>val[i][0];
		}
		const int lgup = ceil(log2(x));
		for(int lg = 1;lg <= lgup;lg++){
			for(int i = 1;i <= x;i++){
				const int nxt = min(x,i + (1<<(lg-1)));
				val[i][lg] = max(val[i][lg-1],val[nxt][lg-1]);
			}
		}
		return ;
	}
	inline int Query(int l,int r){
		const int lg = logval[r - l + 1];
		return max(val[l][lg],val[r - (1<<lg) + 1][lg]);
	}
}st;
