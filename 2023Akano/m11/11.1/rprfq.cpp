...
private:
	struct Node{
		ll maxPos,maxval;
		ll lans,rans,suma,lfarpos,rfarpos;
		ll lazy;
	}node[MAXN * 4];
	inline void AddVal(int p,ll _val){
		node[p].maxval += _val;
		if(node[p].lfarpos != 0){
			node[p].lans += 
		}
	...
	inline pil FindL2R(int p,int l,int r,ll obj,int pos){//某个地点往左边找找到比它小的最高的点
		if(node[p].maxval < obj)return 0;
		if(l == r){//已經有val >= obj
			return {l,(pos - l) * obj};
		}
		const int mid = (l + r) >> 1;
		if(node[p*2].maxval > node[p*2+1].maxval){
			
		}
	}
	inline void PushUp(int p,int l,int r){
		const int mid = (l + r) >> 1,len = r - l + 1;
		if(node[p].maxPos <= mid){//最高点在左边
	...
	void Build(int p,int l,int r){
		if(l == r){
			node[p].maxPos = l,node[p].maxval = node[p].suma = arr[l];
			node[p].lans = node[p].rans = 0;//这个初值0正确吗?
			return ;
		}
		const int mid = (l + r) >> 1;
...
int main(){
	freopen("rprfq/rprfq1.in","r",stdin);
	fio>>tid>>n>>q>>xorval;
	for(int i = 1;i <= n;i++)fio>>arr[i];
	tr.Build();
	for(int i = 1,opt,l,r,x;i <= q;i++){
		fio>>opt;
		if(opt == 1){
			fio>>l>>r>>x;
			l ^= xorval * lastans,r ^= xorval * lastans;
		}else{
			fio>>l>>r;
			l ^= xorval * lastans,r ^= xorval * lastans;
		}
