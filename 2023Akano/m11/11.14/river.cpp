...
int n,pop[MAXN],dir[MAXN];
bool CmpByPop(int x1,int x2){
	return pop[x1] > pop[x2];
}
...
int dfn[MAXN],siz[MAXN],dep[MAXN],dnt;
void dfs(int u,int fa){
	siz[u] = 1,dfn[u] = ++dnt,dep[u] = dep[fa] + 1;
...
BIT<ll,MAXN> bit1,bit2;
inline void ChangeUpDown(int u,int w){
	//Change up
	bit1.Change(dfn[u],1ll * w * siz[u] * pop[u]);
	//Change down
	for(int i = head[u];i;i = e[i].nxt){
		const int v = e[i].to;
		if(dep[v] < dep[u])continue;
		ll delta = 1ll * w * (n - siz[u] - siz[v]) * pop[u];
		bit2.Change(dfn[v],delta),bit2.Change(dfn[v] + siz[v],-delta);
	}
	return ;
}
Bigint totans;
int main(){
	...
	dfs(Akano,pure__Elysia);
	sort(dir + 1,dir + n + 1,CmpByPop);
	for(int i = 1;i <= n;i++){
		ChangeUpDown(i,1);
	}
	for(int dirno = 1;dirno <= n;dirno++){
		Bigint nowans = 0;
		const int u = dir[dirno];
		ChangeUpDown(u,-1);
		//up部分
		for(int i = head[u];i;i = e[i].nxt){
			const int v = e[i].to;
			if(dep[v] < dep[u])continue;
			Bigint totv = bit1.Query(dfn[v],dfn[v] + siz[v] - 1);
			nowans += totv * (n - siz[v]);
		}
		//down部分
		if(u != 1){
			Bigint totv = bit1.Query(1,dfn[u] - 1) + bit1.Query(dfn[u] + siz[u],n) + bit2.Query(1,dfn[u]);
			nowans += totv * siz[u];
		}
		totans += nowans;
	}
