...
莫队问题的一般形式：给定一个序列，对   [katex]  l,r  [/katex]   区间进行询问。  

如果这个信息足够好维护且可合并，直接线段树就可以解决（甚至还可以带修），但是面对某些不是那么好维护或者无法在河里的时空复杂度内维护的信息就要使用莫队了（比如说颜色个数，每个线段树用桶来维护花费太大）  

## 暴力莫队
首先我们还是抛开时间复杂度的问题。对于   [katex]  q  [/katex]   个询问，我们**依次**解决（不排序）。  

这也就是说，我们已经求出了   [katex]  [l,r]  [/katex]   的答案和信息，需要接下来求   [katex]  [l',r']  [/katex]   的答案。显然我们一步步走，只要能求出   [katex]  [l \pm 1,r \pm 1]  [/katex]   就可以算出所有答案了。  

那么具体怎么做呢？这是按照题目而定的。例如上文提到的统计颜色个数（给定一个序列   [katex]  a  [/katex]   ，询问   [katex]  l,r  [/katex]   之间有多少个不同的数），显然用一个桶来维护，那么扩展（`--l`,`++r`）就是往桶里加元素，收缩（`l++`,`r--`）就是往桶里减元素，当一个元素“无中生有”或者“化有为无”后当前答案就会相应加减一。然后就 OK 啦 (*￣︶￣)   

复杂度和指针   [katex]  l,r  [/katex]   的移动次数有关，因为单次移动是   [katex]  O(1)  [/katex]   的（桶的复杂度，如果是线段树之类的还要带上数据结构的复杂度），每个询问最多移动   [katex]  O(n)  [/katex]   次，所以复杂度   [katex]  O(nq)  [/katex]   ，显然不可接受，需要优化。  

## 普通莫队
我们注意到容器的复杂度已经没法优化了（如果需要的话，可以把指针移动的询问离线下来，降低容器带来的复杂度，这是二次离线的内容），需要优化指针的移动次数。  

考虑把每次询问   [katex]  l,r  [/katex]   看做一个点，那么我们需要做的就是在二维平面上生成一颗曼哈顿距离最小生成树，这是一个可行的方案，但是没有必要。莫队采用了更加简洁的方式，对左端点分块，以左端点所在块为第一关键字，右端点为第二关键字。  

那么每次询问的移动，如果左端点和上一个左端点在同一块内，那么左端点移动距离最多为   [katex]  O(\sqrt n)  [/katex]   （实际上是块长，现在取块长为   [katex]  \sqrt n  [/katex]  ），右端点因为是单增的（作为第二关键字排序），单块的均摊复杂度是   [katex]  O(n)  [/katex]   ；如果左端点和和上一个左端点不在同一个块内，就算泉部重新算也不过   [katex]  O(n)  [/katex]   。  

总结一下，（在   [katex]  n,q  [/katex]   同阶的情况下）对于右端点，从每块的角度来看，单块最多   [katex]  O(n)  [/katex]  ，总共   [katex]  O(n \sqrt n)  [/katex]  ；对于左端点，块内部每次   [katex]  O(\sqrt n)  [/katex]   ，块间   [katex]  O(n)  [/katex]  ，总共   [katex]  O(n \sqrt n)  [/katex]  。

## 莫队优化
首先块长并不是卡死   [katex]  \sqrt n  [/katex]   的。分析一下复杂度，实际应该是   [katex]  O(q \times b + \frac n b \times n)  [/katex]   （  [katex]  b  [/katex]   为块长），前者由块内移动贡献，后者由块间移动贡献（  [katex]  \frac n b  [/katex]   就是块的数量）。在   [katex]  b = \frac n {\sqrt q}  [/katex]   的时候，复杂度最小，为   [katex]  O(n \sqrt q)  [/katex]  。所以说在   [katex]  n,q  [/katex]   不同阶的时候要调整块长。例如   [katex]  q = \sqrt n  [/katex]   的时候，应该取   [katex]  b = n ^ {\frac 3 4}  [/katex]   使得复杂度为   [katex]  O(n ^ {\frac 5 4})  [/katex]    
...
## 注意事项
1. 块长不能为   [katex]  0  [/katex]  ，一般来说取不到   [katex]  0  [/katex]   但是还是要注意下。  
2. 扩展收缩(  [katex]  l,r  [/katex]   加加减减前加后减之类的)是有要求的！直接背并不好背，需要理解。这里只介绍咱的习惯。首先扩展都是左自减左自加（`--l`,`++r`），因为   [katex]  l,r  [/katex]   表示的是   [katex]  [l,r]  [/katex]   已经被解决，那么新增的肯定是   [katex]  l-1,r+1  [/katex]   啊。而删除则是要撤掉现在的   [katex]  l  [/katex]   或者   [katex]  r  [/katex]  ，所以是右自减右自加（`l++`,`r--`）。然后先扩展再收缩是因为有时候如果当前指针   [katex]  l \gt r  [/katex]   会出错，所以说先扩展再收缩才不会错
...
还是之前颜色数量统计的问题（不同数字统计），我们将初始序列看做时间   [katex]  t1  [/katex]  ，进行了一次操作将第三个数字从   [katex]  n1  [/katex]   改成   [katex]  n2  [/katex]   之后的序列看做时间   [katex]  t2  [/katex]  。那么我们先进行区间意义上的扩展收缩使得左右指针匹配询问（也许可以把顺序反过来，这里是个人习惯），然后在时间轴上进行穿梭（很酷！）。首先从   [katex]  t1  [/katex]   穿梭到   [katex]  t2  [/katex]   我们当然需要把序列的第三个数字改为   [katex]  2  [/katex]  ，这是废话，然后我们要考虑这次穿梭对已有信息和答案的变化。  

这个例子其实很简单，如果修改的点   [katex]  k  [/katex]   满足   [katex]  l \le k \le r  [/katex]   的话，说明我们的区间包含了这次修改，自然会被这次穿梭影响到，所以我们删除一个数字   [katex]  n1  [/katex]   并加上一个数字   [katex]  n2  [/katex]  （这个操作和平常对序列上的一个数进行增删没有任何区别）。否则，这次穿梭对答案和信息没有影响（当然，序列还是要改的）  

那么时间轴上的收缩操作呢？也只是反操作罢了，当你从   [katex]  t2  [/katex]   返回   [katex]  t1  [/katex]   时，看做执行修改使得   [katex]  k  [/katex]   位置上的   [katex]  n2  [/katex]   变为   [katex]  n1  [/katex]   就行了。
