...
string str;
int n;
ll f[MAXN][MAXN],sumq[MAXN],pow2[MAXN],ans;
int main(){
	...
	for(int len = 2;len <= n;len++){
		for(int l = 1;l + len - 1 <= n;l++){
			const int r = l + len - 1;
			if(str[l] != '(')f[l][r] = (f[l][r] + f[l+1][r]) % MOD;
			if(str[r] != ')')f[l][r] = (f[l][r] + f[l][r-1]) % MOD;
			if(str[l] != '(' && str[r] != ')'){
				f[l][r] = (f[l][r] - f[l+1][r-1] + MOD) % MOD;
			}
			if(str[l] != ')' && str[r] != '('){
				f[l][r] = (f[l][r] + f[l+1][r-1] + pow2[sumq[r-1] - sumq[l+1-1]]) % MOD;
			}
		}
	}
	fio<<f[1][n]<<endl;
