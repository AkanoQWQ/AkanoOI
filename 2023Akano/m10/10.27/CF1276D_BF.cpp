...
ll f[MAXN][3],n,sons[MAXN],pre01[MAXN],suf02[MAXN];
void dfs(int u,int fa,int eval){
	...
	for(int i = 1;i <= maxd;i++){
		f[u][0] += ((pre01[i-1] * f[sons[i]][1]) % MOD) * suf02[i+1];
		f[u][0] %= MOD;
	}
	f[u][1] = (pre01[maxd] * suf02[maxd+1]) % MOD;
	for(int i = maxd + 1;i <= sonTail;i++){
		f[u][2] += ((pre01[i-1] * f[sons[i]][1]) % MOD) * suf02[i+1];
		f[u][2] %= MOD;
	}
	...
	cerr<<u<<" f "<<f[u][0]<<","<<f[u][1]<<","<<f[u][2]<<" maxd "<<maxd<<" in "<<sonTail<<endl;
	return ;
}
int main(){
