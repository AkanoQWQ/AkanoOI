...
namespace HeavyDecomposition{
	...
	inline void Change1(int u,ll w){
		tr.Change(dfn[u],dfn[u] + siz[u] - 1,w);
		return ;
	}
	inline void Change2(int u,int v,ll w){
		while(top[u] != top[v]){
			if(dep[top[u]] < dep[top[v]])swap(u,v);
			tr.Change(dfn[top[u]],dfn[u],w);
			u = fa[top[u]][0];
		}
		if(dep[u] > dep[v])swap(u,v);
		tr.Change(dfn[u],dfn[v],w);
		return ;
	}
};
...
int main(){
	...
	while(q--){
		...
		if(opt == 1){
			cin>>u>>w;
			HeavyDecomposition::Change1(u,w);
		}else if(opt == 2){
			cin>>u>>v>>w;
			HeavyDecomposition::Change2(u,v,w);
		}
		cout<<Solve()<<'\n';
	}
