...
int n,a[TOT_MAXN],b[TOT_MAXN];
inline ll KSM(ll aval,ll bval){
	ll ret = 1;
	while(bval){
		if(bval & 1)ret = (ret * aval) % MOD;
		aval = (aval * aval) % MOD;
		bval >>= 1;
	}
	return ret;
}
inline ll Div(ll aval,ll bval){
	return (aval * KSM(bval,MOD-2)) % MOD;
}
namespace Subtask1{
	const int MAXN = 14;
	const int MAXVAL = 33;
