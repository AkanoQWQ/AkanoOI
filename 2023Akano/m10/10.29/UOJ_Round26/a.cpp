...
public:
	...
	FastIO& operator<<(unsigned long long ullvar){
		WriteInt(ullvar);
		return *this;
	}
...
namespace Subtask1{
	...
	inline bool Check(){
		...
		if(vec[1].empty())return false;
		ull hashval = 10181108;
		for(int i = 1;i <= n;i++){
			hashval *= base1;
			for(auto j : vec[i]){
				hashval = hashval * base2 + j;
			}
		}
		for(int i = 2;i <= n;i++){
			if(vec[i].empty())return false;
			vec[1] = Merge(vec[1],vec[i]);
		}
		for(int i = 1;i <= m;i++){
			if(vec[1][i-1] != a[i])return false;
		}
...
//			return false;
	...
	void dfs(int u){
		...
		for(belong[u] = 1;belong[u] <= n;belong[u]++){
			dfs(u+1);
		}
		return ;
	}
...
int main(){
	freopen("a/ex_merge3.in","r",stdin);
	fio>>n>>m>>o;
	for(int i = 1;i <= m;i++)fio>>a[i];
	if(n <= 4 && m <= 22){
		fio<<Subtask1::Solve()<<endl;
	}else{
		fio<<0<<endl;
	}
