...
inline ll GetG(int x){
	clk -= clock();
	auto last = save.lower_bound(make_pair(x,MOD+1));
	clk += clock();
	ll ret = last->second;
...
//					BF_Change(i,block[i].l,block[i].r);
//					continue;
	...
	cerr<<clk<<" time used of "<<clock()<<endl;
