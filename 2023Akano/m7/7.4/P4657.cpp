...
ll Prefromf[MAXN][MAXV][2],Pretof[MAXN][MAXV][2];
...
void dfs(int u,int fa){
	...
	for(int i = head[u];i;i = e[i].nxt){
		...
		for(int j = 0;j <= V;j++){
			const int k = V - j;
			Update(mans,max(fromf[v][j][0],fromf[v][j][1])+max(Pretof[u][k][0],Pretof[u][k][1]+F[fa]-F[v]));
			Update(mans,max(tof[v][j][0],tof[v][j][1])+max(Prefromf[u][k][0],Prefromf[u][k][1]));
		}
		for(int j = 0;j <= V;j++){
			...
			//Update Presum
			if(j-1>=0)Update(Pretof[u][j][0],max(Pretof[u][j-1][0],tof[u][j][0]));
			if(j-1>=0)Update(Pretof[u][j][1],max(Pretof[u][j-1][1],tof[u][j][1]));
			if(j-1>=0)Update(Prefromf[u][j][0],max(Prefromf[u][j-1][0],fromf[u][j][0]));
			if(j-1>=0)Update(Prefromf[u][j][1],max(Prefromf[u][j-1][1],fromf[u][j][1]));
		}
	}
	return ;
}
int main(){
	cout<<sizeof(fromf) * 4 / 1000 / 1000<<" ?!!!!"<<endl;
