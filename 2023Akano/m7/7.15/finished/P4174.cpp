...
using namespace std;
using ll = long long;
const int MAXN = 80000 + 1018 + 1108;//remember to EDIT
const int MAXM = 500000 + 1018 + 1108;//EDIT and *2
const int MAXR = 5006;
...
int n,m,nnt,VST,VED,cost[MAXR],no[MAXR];
ll sum;
int main(){
	...
	for(int i = 1;i <= m;i++){
		...
		sum += c;
		int VNO = ++nnt;
		CreateEdge(VST,VNO,c);
		CreateEdge(VNO,no[a],INF),CreateEdge(VNO,no[b],INF);
	}
	const ll DNC = Dinic(nnt,VST,VED);
	cout<<sum-DNC;
	return 0;
}
