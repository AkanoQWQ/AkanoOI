...
//void TODP(int x,int y,int stat,int health,double _val){
//	if(dp[x][y][stat][health] >= _val)return ;
//	dp[x][y][stat][health] = _val;
//	for(int k = 0;k < 4;k++){
//		const int OX = x + dx[k],OY = y + dy[k];
//		if(Judge(OX,OY) && (mp[OX][OY] == '.' || mp[OX][OY] == '$'))TODP(OX,OY,stat,health,_val);
//	}
//	return ;
//}
...
//	if(mp[x][y] == '@')return 1;
////	if(dp[x][y][stat][health] >= EPS)return dp[x][y][stat][health];
...
////			dp[x][y][stat][health] = ans1 + ans2;
//			TODP(x,y,stat,health,ans1 + ans2);
////			cout<<"DP "<<x<<","<<y<<","<<Srd(stat)<<","<<health<<" : "<<dp[x][y][stat][health]<<endl;
//			return dp[x][y][stat][health];
//		}else if(copys == 1){
...
////		if(Judge(OX,OY) && vis[OX][OY][stat][health])OK = false;
//	}
////	dp[x][y][stat][health] = ret;
////	vis[x][y][stat][health] = false;
//	TODP(x,y,stat,health,ret);
//	return ret;
//}
//int main(){
