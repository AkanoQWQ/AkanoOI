...
//	ll ret = 1;
//	if(x == 0)return 1;
//	ret *= GetInvWithoutP(x / p,p,pk);
//	ret *= KSM(sumPKINV,x / pk,pk);
//	const ll up = x % pk;
//	for(int i = 1;i <= up;i++){//CAN BE PRE_CALC
//		if(i % p != 0)ret = (ret * i) % pk;
//	}
//	return ret;
//}
...
//	ll sumPTimes = GNOP(n,pk) - GNOP(m,pk) - GNOP(n-m,pk);
//	if(sumPTimes < 0){
