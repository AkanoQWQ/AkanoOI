...
int n,m,art[MAXR][MAXR],sci[MAXR][MAXR],art_same[MAXR][MAXR],sci_same[MAXR][MAXR],VST,VED,no[MAXR][MAXR][2],nnt;
ll sum;
int main(){
	...
	for(int i = 1;i <= n;i++)for(int j = 1;j <= m;j++)cin>>art_same[i][j];
	for(int i = 1;i <= n;i++)for(int j = 1;j <= m;j++)cin>>sci_same[i][j];
	for(int i = 1;i <= n;i++){
		for(int j = 1;j <= m;j++){
			sum += art[i][j] + sci[i][j] + art_same[i][j] + sci_same[i][j];
			const int no_art = ++nnt,no_sci = ++nnt;
			CreateEdge(VST,no[i][j][0],art[i][j]);
			CreateEdge(no[i][j][1],VED,sci[i][j]);
			CreateEdge(VST,no_art,art_same[i][j]);
			CreateEdge(no_sci,VED,sci_same[i][j]);
			CreateEdge(no_art,no[i][j][0],INF);
			CreateEdge(no[i][j][1],no_sci,INF);
			for(int k = 0;k < 4;k++){
				const int OX = i + dx[k],OY = j + dy[k];
				if(OX <= 0 || OX > n || OY <= 0 || OY > m)continue;
				CreateEdge(no_art,no[OX][OY][0],INF);
				CreateEdge(no[OX][OY][1],no_sci,INF);
			}
		}
	}
