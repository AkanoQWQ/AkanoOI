...
const int MAXP =  + 1018 + 1108;//remember to EDIT
const int MAXM =  + 1018 + 1108;//EDIT and *2
...
int main(){
	...
	for(int i = 1;i <= n;i++){
		...
		cin>>query;
		no[i] = ++nnt;
		CreateEdge(no[i],VED,query,0);
	}
	for(int i = 1;i <= m;i++){
		int l,r,c;
		cin>>l>>r>>c;
		pop[i] = ++nnt;
		CreateEdge(VST,pop[i],INF,c);
		for(int j = l;j <= r;j++){
			CreateEdge(pop[i],no[j],INF,0);//INF?
		}
	}
