...
class KDTree{
	private:
		...
		int Build(int l,int r){
			...
			node[p].val = arr[mid];
			node[p].mn[1] = node[p].mx[1] = arr[mid].first;
			node[p].mn[2] = node[p].mx[2] = arr[mid].second;
			if(mid-1 >= l)node[p].l = Build(l,mid-1);
			if(r >= mid+1)node[p].r = Build(mid+1,r);
			if(node[p].l != 0){
				node[p].mn[1] = min(node[p].mn[1],node[node[p].l].mn[1]);
				node[p].mx[1] = max(node[p].mx[1],node[node[p].l].mx[1]);
				node[p].mn[2] = min(node[p].mn[2],node[node[p].l].mn[2]);
				node[p].mx[2] = max(node[p].mx[2],node[node[p].l].mx[2]);
			}
			if(node[p].r != 0){
				node[p].mn[1] = min(node[p].mn[1],node[node[p].r].mn[1]);
				node[p].mx[1] = max(node[p].mx[1],node[node[p].r].mx[1]);
				node[p].mn[2] = min(node[p].mn[2],node[node[p].r].mn[2]);
				node[p].mx[2] = max(node[p].mx[2],node[node[p].r].mx[2]);
			}
			return p;
		}
		ll QueryMn(int p){
