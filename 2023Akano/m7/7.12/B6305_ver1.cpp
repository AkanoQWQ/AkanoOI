...
namespace SLPF{
	class SegmentTree{
		private:
			...
			void Change(int p,int l,int r,int OBJL,int OBJR,int _val){
				if(OBJL <= l && r <= OBJR){
					val[p] = min(val[p],_val),lazy[p] = min(lazy[p],_val);
					return ;
				}
				const int mid = (l + r) >> 1;
				PushDown(p);
	...
	inline void Change(int x,int y,int _val){
		if(dep[x] < dep[y])swap(x,y);//dep_x > dep_y
		while(top[x] != top[y]){
			tr.Change(dfn[top[x]],dfn[x],_val);
			x = fa[top[x]][0];
		}
		if(dep[x] < dep[y])swap(x,y);//dep_x > dep_y
		tr.Change(dfn[y]+1,dfn[x],_val);
		return ;
	}
	inline int Query(int x,int y){
		...
		if(dep[x] < dep[y])swap(x,y);//dep_x > dep_y
		while(top[x] != top[y]){
			ret = min(ret,tr.Query(dfn[top[x]],dfn[x]));
			x = fa[top[x]][0];
		}
		if(dep[x] < dep[y])swap(x,y);//dep_x > dep_y
		ret = min(ret,tr.Query(dfn[y],dfn[x]));
		return ret;
	}
	inline int GetVal(int no){
		int u = e[no].to,v = e[no].to;
...
int main(){
	...
	freopen("./easy/ex_easy2.in","r",stdin);
	freopen("./easy/ex_easy2.out","w",stdout);
	cin>>n>>m;
	for(int i = 1;i <= m;i++){
		int u,v,w;
		cin>>u>>v>>w;
