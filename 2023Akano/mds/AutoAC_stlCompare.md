...
测试数据为插入   [katex]  10^8  [/katex]   个 `int`，在没有 `resize` 的情况下是 `push_back`，有 `resize`的情况下直接访问(调用 `operator[]`)
...
`手写Vector` :   [katex]  0.50 s  [/katex]    

`STL vector` :   [katex]  0.57 s  [/katex]    

人工组略胜一筹   [katex]   15 \%   [/katex]  

### O2 - 有 reserve

`手写Vector` :   [katex]  0.24 s  [/katex]    

`STL vector` :   [katex]  0.35 s  [/katex]    

人工组大胜   [katex]   45 \%   [/katex]  

### O2 - 有 resize

`手写Vector` :   [katex]  0.19 s  [/katex]    

`STL vector` :   [katex]  0.28 s  [/katex]    

人工组大胜   [katex]   47 \%   [/katex]  

### 无O2 - 无 reserve  

`手写Vector` :   [katex]  0.74 s  [/katex]    

`STL vector` :   [katex]  1.31 s  [/katex]    

人工组大获全胜   [katex]   74 \%   [/katex]  

### 无O2 - 有 reserve

`手写Vector` :   [katex]  0.52 s  [/katex]    

`STL vector` :   [katex]  1.14 s  [/katex]    

人工组赢麻了   [katex]   119 \%   [/katex]  

### 无O2 - 有 resize

`手写Vector` :   [katex]  0.40 s  [/katex]    

`STL vector` :   [katex]  0.58 s  [/katex]    
...
然而虽然说理论复杂度   [katex]  O(1)  [/katex]   但是常数巨大而且最坏复杂度   [katex]  O(n)  [/katex]   并且会在 CF 上被卡，有时甚至不如 `map`

测试数据为使用 `operator[]` 访问   [katex]  5 \times 10^7  [/katex]   个 `int` (访问   [katex]  10^8  [/katex]   个数据炸内存了......才发现 `unordered_map` 内存使用巨大)  
...
`STL unordered_map` : 内存使用   [katex]  2.05 GB  [/katex]   (!!!!!)，时间使用   [katex]  5.15  [/katex]   秒

因为我的手写哈希表有一个参数 `size` 表示哈希表头大小，所以有很多组数据。  

**size == 1e6**

`手写 HashMap` : 内存使用   [katex]  0.60 GB  [/katex]  ，时间使用   [katex]  13.53s  [/katex]  

**size == 5e6**

`手写 HashMap` : 内存使用   [katex]  0.62 GB  [/katex]  ，时间使用   [katex]  1.03s  [/katex]  

### 无 O2

`STL unordered_map` : 内存使用   [katex]  2.06 GB  [/katex]  ，时间使用   [katex]  15.44  [/katex]   秒

**size == 1e6**

`手写 HashMap` : 内存使用   [katex]  0.68 GB  [/katex]  ，时间使用   [katex]  22.54s  [/katex]  

**size == 5e6**

`手写 HashMap` : 内存使用   [katex]  0.86 GB  [/katex]  ，时间使用   [katex]  5.08s  [/katex]  
...
发现自己写的 `Deque` 也好费内存啊，数据范围调整到   [katex]  5 \times 10^7  [/katex]   了（不然炸内存）  
...
`STL deque` : 用时   [katex]  0.20s  [/katex]    内存   [katex]  0.21 GB  [/katex]  

`手写 Deque` ： 用时   [katex]  2.00s  [/katex]    内存   [katex]  1.51 GB  [/katex]  
