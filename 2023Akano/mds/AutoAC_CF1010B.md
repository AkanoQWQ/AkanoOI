## 简要题意
给你一个二分猜数字的模板交互题，但是交互器会   [katex]  n  [/katex]   周期性地给出错误的答案，你需要在   [katex]  60  [/katex]   次内猜出范围为   [katex]  [1,10^9]  [/katex]   的数

## 思路
说实话应该是大水题，感觉算是水绿  

首先模板二分咱们都会，但是它的交互器会抽风......观察到   [katex]  \log_2 10^9  [/katex]   差不多刚好是   [katex]  60  [/katex]   的二倍，提示我们要两次询问解决这个问题。  

首先想到先总的求一次答案再用第二次询问更正答案，不过貌似不太可行，在不知道“抽风序列”的情况下答案都是无效的，甚至可能是非法的（二分不出来，因为部分正确部分错误），所以我们必须求出“抽风序列”  

那么我们先询问   [katex]  n  [/katex]   次   [katex]  1  [/katex]  ，显然应该返回   [katex]  1  [/katex]  （如果返回   [katex]  0  [/katex]  ，那么输出答案），否则就是抽风了，记录下来。  
