...
public:
	...
	void Draw(){
		...
		if(center == false){
			...
			int realX = StringRect(content,Rect(sizeX,sizeY)).x;
			int spaceX = (sizeX - realX) / 2;
			outtextrect(pos.x + spaceX,pos.y,sizeX - spaceX,sizeY,content.c_str());
		}
		return ;
	}
	Text() = default;
	Text(POS _pos,int _sizeX,int _sizeY,string _content = "") : UIbasic(_pos,_sizeX,_sizeY){
		content = _content,center = false;
		return ;
	}
};
